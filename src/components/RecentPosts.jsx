      ...
      <div className="container mx-auto">
          ...
          {posts.map((post) => (
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="flex justify-end items-center text-sm text-text-subtle mt-auto pt-4 border-t border-white/10">
                  <div className="flex items-center space-x-1.5">
                    <Calendar size={14} />
                    <span>{new Date(post.created_at).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                  </div>
                </div>
                <AdLink to={`/${post.sections?.slug || 'blog'}/${post.slug}`} className="mt-6 w-full">
                  <Button
                    as="span"
        ...
        <motion.div 
            ...
            <Button asChild className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white px-8 py-3 rounded-full text-base font-semibold">
                <Link to="/recursos-de-ingles">Ver todos los recursos</Link>
            </Button>
        </motion.div>
      </div>
    </section>
