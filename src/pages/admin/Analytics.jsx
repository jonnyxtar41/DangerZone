...
import { getAllPostStats } from '@/lib/supabase/posts';
...
const Analytics = ({ posts, categories, subcategories, sections }) => {
    ...
    const fetchStats = useCallback(async () => {
        try {
            const fetchedStats = await getAllPostStats();
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <Select value={sectionFilter} onValueChange={setSectionFilter}>
                            ...
                            <SelectContent>
                                ...
                                {safeSections.map(sec => <SelectItem key={sec.id} value={String(sec.id)}>{sec.name}</SelectItem>)}
                            </SelectContent>
                        </Select>
                        <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                            ...
                            <SelectContent>
                                ...
                                {safeCategories.map(cat => <SelectItem key={cat.id} value={String(cat.id)}>{cat.name}</SelectItem>)}
                            </SelectContent>
                        </Select>
